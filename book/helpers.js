const SVG = {
  ...
  var telemetry = {
    log: function(endpoint, payload) {
      // post request
      console.log("inside test rustviz_2 answers[0].answer.answer telemetry");
      console.log(payload);
      console.log(payload.answers[0].answer.answer)
      var xhr = new XMLHttpRequest();
      var url = "/submitquiz";
      xhr.open("POST", url, true); // could add authentification info
      xhr.setRequestHeader("Content-Type", "application/json");
      ...
      xhr.send(
          JSON.stringify({
            answer: payload.answers[0].answer.answer,
            ques_id: payload["quizName"],
            quizHash: payload["quizHash"],
